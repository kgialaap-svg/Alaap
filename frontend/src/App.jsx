...
export default function App() {
  ...
  const fetchLiveHistory = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.data) && data.data.length > 0) {
        ...
        localStorage.setItem('allap_milestones_v10', JSON.stringify(combined));
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const savedMilestones = localStorage.getItem('allap_milestones_v10') || localStorage.getItem('allap_milestones_v9') || localStorage.getItem('allap_milestones_v8');
