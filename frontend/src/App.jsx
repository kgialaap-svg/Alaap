...
export default function App() {
  ...
  const fetchLiveHistory = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.data) && data.data.length > 0) {
        ...
        localStorage.setItem('allap_milestones_v11', JSON.stringify(combined));
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const savedMilestones = localStorage.getItem('allap_milestones_v11') || localStorage.getItem('allap_milestones_v10') || localStorage.getItem('allap_milestones_v9');
    ...
    if (savedMilestones) {
      try {
        ...
        if (Array.isArray(parsedMs) && parsedMs.length > 0) {
          ...
          // Ensure default milestones keep up-to-date year & cover image references
          const synced = combined.map(m => {
            const def = CLUB_MILESTONES.find(d => d.id === m.id || d.title?.toLowerCase() === m.title?.toLowerCase());
            return def ? { ...m, year: def.year, image: def.image, photos: def.photos } : m;
          });
